...
document.getElementById(
  "adds"
).innerHTML = `${address[0].locality}, ${address[0].distt},${address[0].state} -${address[0].pin} `;
...
bag_arr.map((ele) => {
  ...
  price.innerHTML = `<span>Rs. ${ele.main_price.toFixed(2)}</span><span> (${
    ele.discount
  }%)</span> <b>Rs. ${ele.discount_price.toFixed(2)}`;
