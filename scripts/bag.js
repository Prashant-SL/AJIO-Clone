...
function display_bag(bag_arr) {
  ...
  bag_arr.map((ele, index) => {
    ...
    saving.innerHTML = `<i class="fas fa-angle-down"></i> <span>Savings : </span> Rs. ${(
      ele.quant *
      (ele.main_price - ele.discount_price)
    ).toFixed(2)}`;
    ...
    main_price.innerHTML = `Rs. ${(ele.quant * ele.main_price).toFixed(2)} (${
      ele.discount
    }%)`;
